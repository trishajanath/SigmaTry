...
const initialState = {
  ...
  ratingMaintenance: undefined,
  ratingFacilities: undefined,
  issuesList: [],
  selectedIssue: null,
};
  ...
  const handleSubmit = async () => {
    if (
      ...
      (state.selectedOptionType === "Feedback" &&
        (state.ratingCleanliness === undefined ||
          state.ratingMaintenance === undefined ||
          state.ratingFacilities === undefined))
    ) {
      Toast.show({
        type: "error",
    ...
    try {
      const Submit = {
        ...
        "survey-maintenance": state.ratingMaintenance,
        "survey-facilities": state.ratingFacilities,
      };
    ...
    <>
      ...
      >
        <View style={styles.container}>
          <Text style={styles.main}>Common Areas Complaint</Text>
          ...
          {state.selectedOptionType === "Feedback" && (
            <>
              ...
              <Text style={styles.label}>Maintenance Rating</Text>
              <View style={styles.ratingContainer}>
                {[1, 2, 3, 4, 5].map((rating) => (
                  <TouchableOpacity
                    key={rating}
                    style={[
                      styles.ratingButton,
                      state.ratingMaintenance === rating && styles.selectedRating,
                    ]}
                    onPress={() => handleRatingSelect("ratingMaintenance", rating)}
                  >
                    <Text
                      style={[
                        styles.ratingText,
                        state.ratingMaintenance === rating && styles.selectedRatingText,
                      ]}
                    >
                      {rating}
                    </Text>
              ...
              <View style={styles.ratingContainer}>
                {[1, 2, 3, 4, 5].map((rating) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.ratingButton,
                      state.ratingFacilities === rating && styles.selectedRating,
                    ]}
                    onPress={() => handleRatingSelect("ratingFacilities", rating)}
                  >
                    <Text
                      style={[
                        styles.ratingText,
                        state.ratingFacilities === rating && styles.selectedRatingText,
                      ]}
                    >
                      {rating}
                    </Text>
