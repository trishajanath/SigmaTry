  ...
  const handleSubmit = async () => {
    if (
      ...
      (state.selectedOptionType === "Feedback" &&
        (state.ratingCleanliness === undefined ||
          state.ratingMachines === undefined ||
          state.ratingService === undefined))
    ) {
      Toast.show({
        type: "error",
    ...
    try {
      const Submit = {
        ...
        "survey-machines": state.ratingMachines,
        "survey-service": state.ratingService,
      };
    ...
    <>
      ...
      >
        <View style={styles.container}>
          <Text style={styles.main}>Laundry Complaint</Text>
          ...
          {state.selectedOptionType === "Feedback" && (
            <>
              ...
              <Text style={styles.label}>Machines Rating</Text>
              <View style={styles.ratingContainer}>
                {[1, 2, 3, 4, 5].map((rating) => (
                  <TouchableOpacity
                    key={rating}
                    style={[
                      styles.ratingButton,
                      state.ratingMachines === rating && styles.selectedRating,
                    ]}
                    onPress={() => handleRatingSelect("ratingMachines", rating)}
                  >
                    <Text
                      style={[
                        styles.ratingText,
                        state.ratingMachines === rating && styles.selectedRatingText,
                      ]}
                    >
                      {rating}
                    </Text>
              ...
              <View style={styles.ratingContainer}>
                {[1, 2, 3, 4, 5].map((rating) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.ratingButton,
                      state.ratingService === rating && styles.selectedRating,
                    ]}
                    onPress={() => handleRatingSelect("ratingService", rating)}
                  >
                    <Text
                      style={[
                        styles.ratingText,
                        state.ratingService === rating && styles.selectedRatingText,
                      ]}
                    >
                      {rating}
                    </Text>
