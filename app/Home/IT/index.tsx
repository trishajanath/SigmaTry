...
const ITIncidentForm = () => {
  ...
  const handleSubmit = async () => {
    if (!state.name || !state.employeeId || !state.department || !state.systemAffected) {
      Toast.show({
        type: "error",
        text1: "Error",
    ...
    try {
      const Submit = {
        ...
        id: state.employeeId,
        issueType: "Complaint",
        issueCat: "IT Department",
        actionItem: "IT Department",
        block: state.department,
        floor: state.systemAffected,
        issueContent: state.errorMessage,
        comments: [
          {
            by: state.employeeId,
    ...
    <PaperProvider>
      <ScrollView style={styles.container}>
        <Card style={styles.card}>
          <Card.Content>
            <Text style={styles.title}>IT Department Incident Report</Text>
            ...
            {/* Incident Details */}
            <TextInput
              label="System/Software Affected"
              value={state.systemAffected}
              onChangeText={(text) => dispatch({ type: "SET_SYSTEM_AFFECTED", payload: text })}
              style={styles.input}
            />
            <TextInput
              label="Error Message"
              value={state.errorMessage}
              onChangeText={(text) => dispatch({ type: "SET_ERROR_MESSAGE", payload: text })}
              style={styles.input}
              multiline
            />
            <TextInput
...
const formReducer = (state, action) => {
  switch (action.type) {
    ...
    case "SET_SYSTEM_AFFECTED":
      return { ...state, systemAffected: action.payload };
