...
const initialState = {
  ...
  ratingCooling: undefined,
  ratingNoise: undefined,
...
function reducer(state: any, action: any) {
  switch (action.type) {
    ...
    case "SET_AC_UNIT_NO":
      return { ...state, acUnitNo: action.payload };
    case "SET_CONTENT":
  ...
  const handleSubmit = async () => {
    if (
      ...
      (state.selectedOptionType === "Feedback" &&
        (state.ratingCooling === undefined ||
          state.ratingNoise === undefined ||
    ...
    try {
      const Submit = {
        ...
        "survey-cooling": state.ratingCooling,
        "survey-noise": state.ratingNoise,
    ...
    <>
      ...
      >
        <View style={styles.container}>
          <Text style={styles.main}>AC Complaint</Text>
          ...
          <Text style={styles.label}>AC Unit Number</Text>
          <TextInput
            style={styles.input}
            placeholder="AC Unit Number"
            value={state.acUnitNo}
            onChangeText={(text) =>
              dispatch({ type: "SET_AC_UNIT_NO", payload: text })
            }
          />
          ...
          {state.selectedOptionType === "Feedback" && (
            <>
              <Text style={styles.label}>Cooling Rating</Text>
              <View style={styles.ratingContainer}>
                {[1, 2, 3, 4, 5].map((rating) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.ratingButton,
                      state.ratingCooling === rating && styles.selectedRating,
                    ]}
                    onPress={() => handleRatingSelect("ratingCooling", rating)}
                  >
                    <Text
                      style={[
                        styles.ratingText,
                        state.ratingCooling === rating && styles.selectedRatingText,
                      ]}
                    >
                      {rating}
                    </Text>
              ...
              <Text style={styles.label}>Noise Level Rating</Text>
              <View style={styles.ratingContainer}>
                {[1, 2, 3, 4, 5].map((rating) => (
                  <TouchableOpacity
                    key={rating}
                    style={[
                      styles.ratingButton,
                      state.ratingNoise === rating && styles.selectedRating,
                    ]}
                    onPress={() => handleRatingSelect("ratingNoise", rating)}
                  >
                    <Text
                      style={[
                        styles.ratingText,
                        state.ratingNoise === rating && styles.selectedRatingText,
                      ]}
                    >
                      {rating}
                    </Text>
              ...
              <View style={styles.ratingContainer}>
                {[1, 2, 3, 4, 5].map((rating) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.ratingButton,
                      state.ratingCleanliness === rating && styles.selectedRating,
                    ]}
                    onPress={() => handleRatingSelect("ratingCleanliness", rating)}
                  >
                    <Text
                      style={[
                        styles.ratingText,
                        state.ratingCleanliness === rating && styles.selectedRatingText,
                      ]}
                    >
                      {rating}
                    </Text>
